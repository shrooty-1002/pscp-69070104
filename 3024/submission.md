...
```text
ก็คือ โจทจะให้เราเช็คคะแนนโหวต และถ้าคะแนนต่ำสุดและสูงสุดห่างกันเกิน2 คะแนน ให้พิมว่า surprising ถ้าไม่เกินพิมพ์ Not Surprising
```
...
```
นั่งทำเองและก็ทำไปเรื่อยๆ พอบางส่วนนึกไม่ออกก็เลยถามเพื่อน
---
...
```text
คือตอนทที่เห็นโจทก็คิดออกแล้วว่าต้องเป็น total - (2*max_score) แต่ไม่มั่นใจว่าเอาไปใช้ยังไง เลยถามเพื่่อนว่า สูตรนี้ถูกไหม ละก็ต้องเติมตรงไหนบ้าง เขาก็บอกว่าใส่ max(0.0 , )เติมไปด้วย เพื่อที่คะแนนจะไม่ไปตรงติดลบ และค่าก็จะออกมาถูก 
```

คุณยังทำอะไรด้วยตนเอง

```text
ทำเองเขียนเองหมด แค่ตรง min_score มีการแอบถามเพื่อน
```
